...
- **Frontend:** Startseite (`/`) legt Partien an und listet eigene; `/game/:gameId` zeigt Brett mit Figuren, die sich per Zeiger ziehen lassen und die vor dem Senden gegen die Engine geprüft werden; eine beendete Partie sperrt das Brett und zeigt das Ergebnis. Züge des Gegners kommen live über WebSocket ([ADR-0026](adr/0026-stomp-websocket-statt-rsocket-fuer-zug-push.md)).
- **Sicherheit und Spieler:** Jede Partie hat ein Owner- und ein Invite-Token statt eines Logins; das Backend prüft bei jedem Zug Rolle und Zugrecht anhand des Tokens ([ADR-0025](adr/0025-owner-invite-token-als-partie-identitaet.md)). `SecurityFilterChain` bleibt permit-all ([ADR-0015](adr/0015-security-von-tag-eins.md)) — die Autorisierung liegt in der Anwendungsschicht, nicht in Spring Security.
