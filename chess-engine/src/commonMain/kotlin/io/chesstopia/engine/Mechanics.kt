 ...
 *
 * Gerechnet wird auf `Map<Square, Piece>` — dieselbe Sicht wie die Backend-Domäne.
 */
...
internal fun Position.rejectReason(move: Move): String? {
    ...
    val pieces = pieceMap()
    val piece = pieces[move.from] ?: return "Auf ${move.from} steht keine Figur"
    if (piece.isWhite() != (sideToMove == Color.WHITE)) {
        return "${move.from} gehört nicht der Seite am Zug"
    }
    val target = pieces[move.to]
...
internal fun Position.play(move: Move): Position {
    val before = pieceMap()
    val piece = before.getValue(move.from)
    ...
    if (piece.type == PieceType.PAWN && enPassantTarget == move.to && move.to !in before) {
    ...
    if (piece.type == PieceType.PAWN && move.promotion != null) {
        next[move.to] = Piece(move.promotion, piece.color)
    }
    ...
    if (piece.type == PieceType.KING && abs(move.from.file.ordinal - move.to.file.ordinal) == 2) {
        ...
        val rookFrom = Square(if (kingside) File.H else File.A, rank)
        val rookTo = Square(if (kingside) File.F else File.D, rank)
        next.remove(rookFrom)?.let { next[rookTo] = it }
    }
    ...
    val captured = move.to in before || enPassantCapture

    return Position(
        board = next.map { PlacedPiece(it.key, it.value) }.toTypedArray(),
