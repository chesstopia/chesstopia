...
5. **Kein Lombok.** MapStruct ist für die Adapter-Mapper aufgenommen ([ADR-0021](docs/adr/0021-mapstruct-fuer-adapter-mappings.md)) — `@Mapper(componentModel = "spring")`, `unmappedTargetPolicy=ERROR` für `PositionJsonMapper`, `WebMapper` und `EngineMapper` (Engine-JVM-Build mit `-java-parameters`, damit MapStruct die Kotlin-`data class`-Konstruktoren per Parametername trifft). Nur `GameEntityMapper` bleibt von Hand — die JPA-Entities sind package-private. Die Domäne selbst bleibt Records ohne Framework.
