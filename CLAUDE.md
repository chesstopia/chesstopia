...
- Kollaborateure eines Unit-Tests werden mit Mockito gemockt (`@ExtendWith(MockitoExtension.class)`, `@Mock`, `@InjectMocks`), nicht von Hand als Fake gebaut. Mockito kommt über `spring-boot-starter-test`. Stubs stehen pro Test in der `// ARRANGE`-Phase (Strict Stubs).
...
- Welche Testebene ein Feature braucht, entscheidet [ADR-0019](docs/adr/0019-teststrategie.md); ausgeführt wird das vom Skill `/tests`. Ebene 4 liegt in `e2e/`: handgeschriebene Playwright-Specs unter `e2e/tests/`, Schachsituationen als Dateien unter `e2e/testcases/` ([ADR-0024](docs/adr/0024-datei-getriebener-e2e-korpus.md)).
